...
final class CustomBehaviorViewController: UIViewController {
    ...
    private lazy var keyboardController: KeyboardController = { [unowned self] in
        return KeyboardController(viewController: self, constraint: self.contentHeight, behaviorType: .custom(calculationClosure: self.keyboardDeltaCalculation), scrollView: scrollView)
        }()
